...
woocommerce_add_to_cart_variations_with_radio_get_all_combinations = function () {
  var available_combinations, order, figureOutWhyOptionIsUnavailable = function () {
      var loopThroughAttribute = function (info, currentStep, currentTree) {
          ...
          var currentOrder = order[currentStep], nextTreeStep;
          ...
          if (currentOrder.attrName === info.blockedAttr.attrName) {
            ...
            nextTreeStep = [
              info.blockedAttr.attrName,
              info.blockedAttr.val
            ];
            ...
            nextTreeStep = [
              currentOrder.attrName,
              info.selected[currentOrder.attrName].val
            ];
          }
      ...
      return function (blockedAttr, selected) {
        ...
        var currentAttr, blockList = [], currentSelected;
      ...
      var i = 0, currentOrder, tree = available_combinations, nextTreeName;
      while (i < order.length) {
        currentOrder = order[i];
        if (!currentOrder.alwaysTrue) {
          if (currentOrder.attrName === checkedAttr.attrName) {
            nextTreeName = checkedAttr.val;  // console.log ( "checked", nextTreeName, 123 );
          } else {
            nextTreeName = selected[currentOrder.attrName].val;  // console.log( "static", nextTreeName, 123 );
          }
        } else {
          nextTreeName = currentOrder.attrName;  // console.log( "always true", nextTreeName, 123 );
        }
  ...
  return function (all_variations) {
    var first = true, i = all_variations.length - 1, current_attributes, current_attributeName, current_attributeValue, current_attributeClassName, lastStep = {};
    ...
    while (i >= 0) {
      ...
      for (current_attributeName in current_attributes) {
        ...
        if (first) {
          order.push({
            attrName: current_attributeName,
            alwaysTrue: current_attributeValue === current_attributeName
          });
        }
      }
      first = false;
...
woocommerce_add_to_cart_variations_with_radio = function (getAllCombinations) {
  ...
  $.fn.betoniuWcVariationForm = function () {
    var $form = this, $fieldsets = this.find('fieldset'), $inputWrappers = this.find('.product_variable_option'), $formVariationId = this.find('input.variation_id'), $product = $(this).closest('.product'), $imageDiv = $product.find('.images'), $product_img = $imageDiv.find('img:eq(0)'), o_src = $product_img.attr('src'), o_title = $product_img.attr('title'), o_alt = $product_img.attr('alt'), attributes = {}, values = {}, allVariations = $form.data('product_variations') || window.product_variations || window['product_variations_' + parseInt($form.data('product_id'))], initCheckSingleFieldset = function (index, wrapper) {
        var $fieldset = $(wrapper), niceName = $fieldset.find('legend').html(), attrName = $fieldset.data('attribute_name') || $fieldset.attr('name'), linkedData = {};
        ...
        var currentSettings, currentSettingsDetailed, checkFieldsetsAndFindCurrentSettings = function () {
            var $this = $(this), checked = $this.find($('input:checked')), fieldValue = checked.val(), attribute_name;
        ...
        return function (event, exclude, focus) {
          var matching_variations, product_id = parseInt($form.data('product_id')), variation, $variation_input;
          currentSettings = {};
          ...
          if (variation) {
            // Set ID
            $formVariationId.val(variation.variation_id).change();
        ...
        return function (event, selectedValues) {
          ...
          $inputWrappers.each(function (index, el) {
            var data = el.linkedData, $input = data.$input, $tooltip = data.$tooltip, description = data.description;
            if (checkCombinations.checkIfPossible({
              ...
              $tooltip.attr('disabled', false);
              $tooltip.html('Nicht verf\xFCgbar f\xFCr: ' + checkCombinations.figureOutWhyOptionIsUnavailable(data, selectedValues).join(', '));
            }
          });
        };
      }(), changeImage = function (variation) {
        var variation_image = variation.image_src, variation_link = variation.image_link, variation_title = variation.image_title, variation_alt = variation.image_alt;
        ...
        var $variationButton = $form.find('.variations_button'), $single_variation = $form.find('.single_variation'), $single_variation_wrap = $form.find('.single_variation_wrap'), $sku = $product.find('.product_meta').find('.sku'), $weight = $product.find('.product_weight'), $dimensions = $product.find('.product_dimensions'), $quantity = $single_variation_wrap.find('.quantity'), $inputQuantity = $quantity.find('input.qty');
        ...
        return function (event, variation) {
          ...
          $sku.text(variation.sku ? variation.sku : $sku.attr('data-o_sku'));
          $weight.text(variation.weight ? variation.weight : $weight.attr('data-o_weight'));
          $dimensions.text(variation.dimensions ? variation.dimensions : $dimensions.attr('data-o_dimensions'));
          ...
          if (!variation.variation_is_visible) {
            $single_variation.html('<p>Diese Variante ist leider nicht verf\xFCgbar.</p>');
          } else {
    ...
    $.fn.betoniuWcVariationForm.find_matching_variations = function (product_variations, settings) {
      var matching = [], variation, variation_id;
    ...
    $.fn.betoniuWcVariationForm.variations_match = function (attrs1, attrs2) {
      var match = true, attr_name, val1, val2;
